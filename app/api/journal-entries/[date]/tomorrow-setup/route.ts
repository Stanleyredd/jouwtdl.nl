...
import {
  normalizeTomorrowSetupPayload,
  saveTomorrowSetupForUser,
...
export async function PATCH(
  ...
  try {
    payload = normalizeTomorrowSetupPayload(
    ...
    return Response.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Use a valid tomorrow setup payload.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    const { journalEntry } = await saveTomorrowSetupForUser(
      context.user.id,
      date,
      payload,
    );
