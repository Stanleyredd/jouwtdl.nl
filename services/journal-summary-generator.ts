...
import { getEnabledJournalConfigSections } from "@/lib/journal-config";
...
export async function generateJournalSummaryFromStoredContent({
  date,
  journalText,
  sections,
  tomorrowSetup,
  ...
  if (!date) {
    throw new Error("A journal date is required.");
  }

  if (!journalText.trim()) {
    throw new Error("Write a little more before generating a summary.");
  }
  ...
  const prompt = buildSummaryPrompt(
    date,
    journalText,
    normalizeTomorrowSetup(tomorrowSetup),
    language,
    journalConfig,
  );
  ...
  for (const attempt of [
    ...
    if (process.env.NODE_ENV === "development") {
      console.debug("[journal-summary]", "raw-summary-response-received", {
        ...
        isIncomplete: Boolean(response.incomplete_details),
      });
    }
